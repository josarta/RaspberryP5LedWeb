...
class OLED:
    def __init__(self, bus_number=1, i2c_address=0x3C, rotate_angle=0, **kwargs):
    ...
    def draw_text(self, text, position=(0, 0), font_size=None, directory="left"):
        # Display text in the buffer
        if font_size is None or not os.path.exists(self.default_font_path):
            font = self.font
        else:
            try:
        ...
        pos = position
        if isinstance(position, (tuple, list)) and len(position) == 2 and isinstance(position[0], (tuple, list)):
            pos = (position[0][0], position[0][1])

        self.draw.text(pos, str(text), font=font, fill="white")
